...
function FormEndereco({ form, setForm }) {
  ...
  return (
    <>
      ...
      <div className="row2">
        <div className="field"><label>CEP</label><input value={form.endereco.cep} onChange={e => setEndereco('cep', e.target.value)} placeholder="00000-000" maxLength={9} /></div>
        <div className="field"><label>Tipo logradouro</label><input value={form.endereco.tipo_logradouro} onChange={e => setEndereco('tipo_logradouro', e.target.value)} placeholder="Rua, Av..." /></div>
      </div>
      <div className="row3">
        <div className="field" style={{ gridColumn: 'span 2' }}><label>Logradouro</label><input value={form.endereco.logradouro} onChange={e => setEndereco('logradouro', e.target.value)} placeholder="Nome da rua" /></div>
        <div className="field"><label>Número</label><input type="number" value={form.endereco.numero} onChange={e => setEndereco('numero', e.target.value)} placeholder="100" /></div>
      </div>
      <div className="field"><label>Complemento</label><input value={form.endereco.complemento} onChange={e => setEndereco('complemento', e.target.value)} placeholder="Sala, bloco, apto..." /></div>
      <div className="row2">
        <div className="field"><label>Município</label><input value={form.endereco.municipio} onChange={e => setEndereco('municipio', e.target.value)} placeholder="São Paulo" /></div>
        <div className="field"><label>Estado</label>
          <select value={form.endereco.estado} onChange={e => setEndereco('estado', e.target.value)}>
...
export default function Agencias() {
  ...
  const columns = useMemo(() => [
    ...
    {
      id: 'municipio', header: 'Município',
      accessorFn: row => row.endereco?.municipio ?? '—',
    },
    {
      id: 'estado', header: 'Estado', size: 80,
      accessorFn: row => row.endereco?.estado ?? '—',
    },
    {
  ...
  return (
    <div className="page">
      <style>{`
        ...
        .modal-overlay { position: fixed; inset: 0; background: rgba(0,0,0,.45); display: flex; align-items: center; justify-content: center; z-index: 50; padding: 1rem; overflow-y: auto; }
        .modal { background: #fff; border-radius: 12px; width: 100%; max-width: 480px; box-shadow: 0 20px 60px rgba(0,0,0,.15); margin: auto; }
        ...
        .modal-body { padding: 1.25rem; display: flex; flex-direction: column; gap: 12px; max-height: 70vh; overflow-y: auto; }
        ...
        .field input, .field select, .field textarea { width: 100%; padding: 8px 10px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 14px; box-sizing: border-box; outline: none; font-family: inherit; }
        .field input:focus, .field select:focus, .field textarea:focus { border-color: #6366f1; box-shadow: 0 0 0 3px rgba(99,102,241,.15); }
        .field textarea { resize: vertical; min-height: 70px; }
        .section-label { font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: .06em; color: #9ca3af; border-top: 1px solid #f3f4f6; padding-top: 12px; margin-top: 4px; }
        ...
        .row3 { display: grid; grid-template-columns: 1fr 1fr 80px; gap: 12px; }
      `}</style>

      <div className="page-header">
        <h2 className="page-title dark:text-white">Agências</h2>
        <div className="toolbar">
          <div className="search-box">
            ...
            <input className='text-slate-700 dark:text-slate-300' placeholder="Buscar agências..." value={globalFilter} onChange={e => setGlobalFilter(e.target.value)} />
          </div>
            ...
            <table>
              ...
              <tbody>
                {table.getRowModel().rows.length === 0 ? (
                  <tr className="empty-row"><td colSpan={columns.length}>Nenhuma agência encontrada.</td></tr>
                ) : (
      ...
      {modal === 'criar' && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && fecharModal()}>
          <div className="modal">
            <div className="modal-header"><h3 className="modal-title">Nova agência</h3><button className="modal-close" onClick={fecharModal}>×</button></div>
            <div className="modal-body">
              {erro && <div className="erro-msg">{erro}</div>}
              <div className="field"><label>Nome</label><input value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} placeholder="Nome da agência" /></div>
              <div className="field"><label>Descrição</label><textarea value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} placeholder="Descreva a agência..." /></div>
              <FormEndereco form={form} setForm={setForm} />
            </div>
            <div className="modal-footer">
              <button className="btn-cancelar" onClick={fecharModal}>Cancelar</button>
              <button className="btn-salvar" onClick={salvarCriar} disabled={salvando}>{salvando ? 'Salvando…' : 'Criar agência'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === 'editar' && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && fecharModal()}>
          <div className="modal">
            <div className="modal-header"><h3 className="modal-title">Editar agência</h3><button className="modal-close" onClick={fecharModal}>×</button></div>
            <div className="modal-body">
              {erro && <div className="erro-msg">{erro}</div>}
              <div className="field"><label>Nome</label><input value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} /></div>
              <div className="field"><label>Descrição</label><textarea value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} /></div>
              <FormEndereco form={form} setForm={setForm} />
            </div>
            <div className="modal-footer">
              <button className="btn-cancelar" onClick={fecharModal}>Cancelar</button>
      ...
      {modal === 'excluir' && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && fecharModal()}>
          <div className="modal">
            <div className="modal-header"><h3 className="modal-title" style={{ color: '#dc2626' }}>Excluir agência</h3><button className="modal-close" onClick={fecharModal}>×</button></div>
            <div className="modal-body">
              {erro && <div className="erro-msg">{erro}</div>}
              <p className="excluir-aviso">Tem certeza que deseja excluir a agência <strong>{selecionado?.nome}</strong>?<br />Esta ação não pode ser desfeita.</p>
            </div>
            <div className="modal-footer">
              <button className="btn-cancelar" onClick={fecharModal}>Cancelar</button>
