...
export default function Usuarios() {
  ...
  const abrirEditar = (usuario) => {
    ...
    setForm({ nome: usuario.nome, email: usuario.email, username: usuario.username, telefone: usuario.telefone ?? '', senha: '' });
    setErro(''); setModal('editar');
  };

  const abrirExcluir = (usuario) => { setSelecionado(usuario); setErro(''); setModal('excluir'); };
  ...
  const salvarCriar = async () => {
    if (!form.nome || !form.email || !form.username || !form.senha) {
      setErro('Nome, e-mail, username e senha são obrigatórios.'); return;
    }
    setSalvando(true);
    try {
      await api.post('/usuarios', form);
      fecharModal(); carregar();
    } catch (e) { setErro(e.response?.data?.message ?? 'Erro ao criar usuário.'); }
  ...
  const confirmarExcluir = async () => {
    ...
    try {
      ...
      fecharModal(); carregar();
    } catch (e) { setErro(e.response?.data?.message ?? 'Erro ao excluir usuário.'); }
    finally { setSalvando(false); }
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Usuários</h1>
      ...
      {modal === 'criar' && (
        <Modal onClose={fecharModal}>
          <ModalHeader title="Novo usuário" onClose={fecharModal} />
          <div className="px-5 py-4 flex flex-col gap-3">
            ...
            <div>
              ...
              <input className={inputCls} value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} placeholder="ex: joao.silva" />
            </div>
            <div>
            ...
            <div>
              ...
              <input className={inputCls} value={form.telefone} onChange={e => setForm(f => ({ ...f, telefone: e.target.value }))} placeholder="(00) 00000-0000" />
            </div>
            <div>
              <label className={labelCls}>Senha</label>
              <input className={inputCls} type="password" value={form.senha} onChange={e => setForm(f => ({ ...f, senha: e.target.value }))} placeholder="Senha inicial" />
            </div>
          </div>
          <ModalFooter>
            <BtnCancelar onClick={fecharModal} />
            <BtnSalvar onClick={salvarCriar} disabled={salvando}>{salvando ? 'Salvando…' : 'Criar usuário'}</BtnSalvar>
          </ModalFooter>
        </Modal>
      )}
      ...
      {modal === 'editar' && (
        <Modal onClose={fecharModal}>
          <ModalHeader title="Editar usuário" onClose={fecharModal} />
          <div className="px-5 py-4 flex flex-col gap-3">
            {erro && <ErroBanner msg={erro} />}
            <div>
              <label className={labelCls}>Username</label>
              <input className={inputDisabledCls} value={form.username} disabled />
              <p className={hintCls}>O username não pode ser alterado.</p>
            </div>
            <div>
              <label className={labelCls}>Nome</label>
            ...
            <div>
              <label className={labelCls}>
                Nova senha <span className="font-normal text-gray-400">(opcional)</span>
              </label>
              <input className={inputCls} type="password" value={form.senha} onChange={e => setForm(f => ({ ...f, senha: e.target.value }))} placeholder="Deixe em branco para manter" />
              <p className={hintCls}>Preencha apenas se quiser alterar a senha.</p>
            </div>
          </div>
          <ModalFooter>
            <BtnCancelar onClick={fecharModal} />
      ...
      {modal === 'excluir' && (
        <Modal onClose={fecharModal}>
          <ModalHeader title="Excluir usuário" danger onClose={fecharModal} />
          <div className="px-5 py-4 flex flex-col gap-3">
            {erro && <ErroBanner msg={erro} />}
            <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
              Tem certeza que deseja excluir o usuário <strong className="text-gray-900 dark:text-gray-100">{selecionado?.nome}</strong>?<br />
